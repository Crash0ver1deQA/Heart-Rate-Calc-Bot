...
async def get_age(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if age < 1 or age > 150:
            await update.message.reply_text("⚠️ Пожалуйста, введите возраст в пределах от 1 года до 150 лет:")
            return AGE
        ...
        await update.message.reply_text("⚠️ Пожалуйста, введите корректный возраст:")
        return AGE
...
async def get_resting_hr(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if resting_hr < 20 or resting_hr > 100:
            await update.message.reply_text("⚠️ Пожалуйста, введите ЧСС в покое в пределах от 20 до 100 уд./мин.:")
            return RESTING_HR
        ...
        response = (
            ...
            f'🟢 Зона 1 (50-60%): {zone_1[0]} - {zone_1[1]} уд./мин. (лёгкая активность)\n'
            f'🔵 Зона 2 (60-70%): {zone_2[0]} - {zone_2[1]} уд./мин. (жиросжигание)\n'
            f'🟡 Зона 3 (70-80%): {zone_3[0]} - {zone_3[1]} уд./мин. (аэробная выносливость)\n'
            f'🟠 Зона 4 (80-90%): {zone_4[0]} - {zone_4[1]} уд./мин. (анаэробный порог)\n'
            f'🔴 Зона 5 (90-100%): {zone_5[0]} - {zone_5[1]} уд./мин. (максимальная нагрузка)'
        )
        ...
        await update.message.reply_text(
            "🔄 Если хотите начать заново, нажмите на кнопку ниже ⬇️",
        ...
        await update.message.reply_text("⚠️ Пожалуйста, введите корректную ЧСС в покое:")
        return RESTING_HR
