...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == "restart":
        # Красиво убираем кнопку (редактируем сообщение)
        await query.edit_message_text(
            text="Расчёт перезапущен! 🔄",
            reply_markup=None
        )
        # Отправляем новое сообщение с приглашением ввести возраст
        await query.message.reply_text("Введите свой возраст (в годах):")
        return AGE  # Переходим в состояние ожидания возраста
...
def main():
    ...
    # ConversationHandler с allow_reentry=True
    conv_handler = ConversationHandler(
        ...
        allow_reentry=True,  # Обязательно для перезапуска по кнопке
    )
